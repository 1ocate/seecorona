...
table = world_data.find(id="main_table_countries_yesterday").find("tbody")
#table = world_data.find(id="main_table_countries_yesterday2").find("tbody")
...
for tr in trs[8:]:
  ...
  row= cursor.fetchone()
  if not row:
     cursor.execute('INSERT INTO world (world,date) VALUES(?,?)',(world,yesterday))

  cursor.execute('UPDATE world SET positive=?, cure=?, death=? WHERE Date=? and world=?',(positive, cure, death, yesterday, world))
  db.commit()
